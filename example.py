...
def example_artistic_effect():
    print("=" * 60)
    print("示例 2: 艺术效果 - 16 色 + K-Means + 抖动")
    print("=" * 60)

    sample_dir = Path("./samples")
    input_path = sample_dir / "sample_gradient.png"
    output_path = sample_dir / "sample_16color_kmeans_fs.png"
    ...
    print(f"输出图片: {output_path}")
    print(f"实际颜色数: {info['unique_colors']}")
    print(f"算法: {info['method']}")
    print(f"抖动: {info['dither']}")
    print()


def example_custom_palette():
    print("=" * 60)
    print("示例 3: 使用自定义调色板 (复古游戏风格)")
    print("=" * 60)

    sample_dir = Path("./samples")
    input_path = sample_dir / "sample_gradient.png"
    output_path = sample_dir / "sample_retro_palette.png"
...
def example_batch_processing():
    ...
    output_dir = sample_dir / "batch_output"
    ...
    quantizer = ColorQuantizer(colors=64, method=QuantizeMethod.MAX_COVERAGE)
    ...
    for r in results:
        if "error" in r:
            ...
            print(f"  {r['filename']}: {r['size_reduction_pct']:.1f}% 压缩率, {r['unique_colors']} 色")
    print()
